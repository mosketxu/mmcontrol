                ...
                <div>
                    <div class="flex w-full py-1 text-sm font-bold text-gray-500 bg-blue-100 rounded-t-md">
                        <div class="flex w-11/12 ">
                            ...
                            <div class="w-1/12 ">{{ __('F.Ini') }}
                                @if($orden=='asc' && $ordenarpor=='fecha_ini')
                                    <x-icon.sort-down-a wire:click="ordenar('fecha_ini')" class="pl-1" title="Orden"/>
                                @elseif (($orden=='desc' && $ordenarpor=='fecha_ini'))
                                    <x-icon.sort-up-a wire:click="ordenar('fecha_ini')" class="pl-1" title="Orden"/>
                                @else
                                    <x-icon.sort-a wire:click="ordenar('fecha_ini')" class="pl-1" title="Orden"/>
                                @endif
                            </div>
                        </div>
                        ...
                        <div class="hover:bg-gray-100 hover:cursor-pointer">
                            <div class="flex items-center w-full text-sm text-gray-500 border-t-0 border-y" wire:loading.class.delay="opacity-50">
                                <div class="flex items-center w-11/12" onclick="location.href = '{{ route('entidad.edit',$entidad) }}'">
                                    ...
                                    <div class="w-1/12 ">{{ $entidad->fecha_ini_vista }}</div>
                                </div>
                                <div class="items-center w-1/12 text-center ">
                                    ...
                                    <x-icon.clipboard-a style="color: #f97316;" href="{{ route('entidad.acciones',[$entidad,'i']) }}" title="Acciones"/>
                                    <x-icon.delete-a wire:click.prevent="delete({{ $entidad->id }})" onclick="confirm('Estas seguro?') || event.stopImmediatePropagation()" class="w-6"/>
                                </div>
                            </div>
                        </div>
                        @empty
