    ...
    @if(!$escliente)
        <form wire:submit.prevent="save">
            <div class="flex w-full py-0 my-0 space-x-1 text-left bg-green-100 border-t-0 border-y" wire:loading.class.delay="opacity-50">
                ...
                {{-- titulo --}}
                <div class="w-2/12">
                    <input type="text" wire:model.live="titulo" placeholder="Introduce el titulo"
        ...
        <div class="flex w-full py-0 my-0 space-x-1 text-left bg-white border-t-0 border-y" wire:loading.class.delay="opacity-50">
            <div class="w-1/12">
                <input type="number" value="{{ $odetalle->orden }}"
                    ...
                    class="w-full py-1 text-xs font-thin text-gray-500 border-0 rounded-md" {{$escliente}} {{$deshabilitado}}/>
                </div>
            <div class="w-2/12">
            ...
            <div class="w-2/12">
                ...
                class="w-full py-1 pr-2 text-xs font-thin text-right text-gray-500 border-0 rounded-md"{{$escliente}} {{$deshabilitado}}/>
            </div>
            <div class="w-1/12">
                <input type="number" step="any" value="{{ $odetalle->importe }}"
                wire:change="changeValor('{{ $odetalle->id }}','importe',$event.target.value)"
                class="w-full py-1 mr-2 text-xs font-thin text-right text-gray-500 border-0 rounded-md"{{$escliente}} {{$deshabilitado}}/>
            </div>
            <div class="w-1/12">
