                ...
                <div>
                    <div class="">
                        ...
                        <div class="flex w-full pt-2 pb-0 pl-2 text-xs font-bold text-gray-500 bg-blue-100 rounded-t-md">
                            ...
                            @if($tipo=='1')
                                <div class="w-4/12 text-left" >{{ __('Isbn/Título') }} </div>
                            @else
                                <div class="w-4/12 text-left" >{{ __('Referencia') }} </div>
                            @endif
                    ...
                    <div>
                        ...
                        <div class="flex items-center w-full py-1 pl-2 text-xs font-medium text-gray-500 border-t-0 border-y hover:bg-gray-100" wire:loading.class.delay="opacity-50">
                            ...
                            {{ $tipo }}
                            @if($tipo=='1')
                                <div class="flex w-4/12 text-left">{{ $oferta->ofertaproducto->isbn }} {{ $oferta->ofertaproducto->referencia }}</div>
                            @else
                                <div class="flex w-4/12 text-left">{{ $oferta->descripcion }}</div>
                            @endif
                            ...
                            {{-- space-x-2 mx-auto justify-between --}}
                            <div class="w-1/12 text-center space-x-1">
                                <x-icon.edit-a class="mb-0.5" href="{{ route('oferta.editar',[$oferta,'i']) }}"  title="Editar"/>
