...
<th {{ $attributes->merge(['class' => 'px-1 py-3 bg-blue-50']) }}>
    ...
    @else
        <button {{ $attributes->except('class') }} class="flex items-center space-x-1 text-xs font-medium leading-4 tracking-wider text-left text-gray-500 group focus:outline-none focus:underline">
            <span>{{ $slot }}</span>
